...
```{r}
pm2 <- ggpubr::ggarrange(plotlist = lapply(list(ptax6, ptax2, pca1p, pca2p), function(p){p+theme(plot.margin = unit(c(1,1,1,1), "cm"))}), labels=c("A", "B", "C", "D"), ncol=2, nrow=2)
pm2
...
phyloseq::psmelt(subset_taxa(physeq, Genus == "Bacteroides")) %>%
  ...
  geom_boxplot(outlier.shape  = NA) +
  geom_jitter(aes(color = Genus), height = 0, width = .2, alpha=0.1) +
  labs(x = "", y = "Abundance\n") +
  scale_x_discrete(labels= c(`0`="Healthy Cohort", `1`="ACVD Cohort")) +
  ...
  theme(axis.title = element_text(size=16),
        ...
        strip.text.x = element_text(size=15)) -> ptax8
ptax8
