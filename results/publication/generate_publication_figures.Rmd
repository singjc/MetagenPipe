...
<!-- Helper Functions -->
```{r}
do_lm <- function(dt_sub, grouped_var, value_col="value", group_col="ACVD", group_ctrl="0", group_treat="1", use_model=lm, family=NULL){
  ...
  # 
  # ggplot(dt_sub, aes(x=!!rlang::sym(value_col), col=as.factor(!!rlang::sym(group_col)))) +
  #       geom_density()
  ...
  if ( (length(ctrl) > 1 & length(treat) > 1) & !all(is.na(ctrl)) & !all(is.na(treat)) ){
    lm.out <- use_model(value ~ as.factor(ACVD), dt_sub, family=family)
...
ggplot(combined_df, aes(x=depth, y=total_feats)) +
...
```{r}
ggplot(stats_df, aes(x=depth, y=union_feats)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
```{r}
ggplot(stats_df, aes(x=depth, y=common_feats)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
```{r}
ggplot(stats_df, aes(x=depth, y=jaccard)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
```{r}
ggplot(stats_df, aes(x=depth, y=cor)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
ggplot(seed_combined_df, aes(x=seed, y=total_feats)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
#### Overlap + Correlation of Features at Varying Depths
```{r}
ggplot(seed_stats_df, aes(x=seed, y=union_feats)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
```{r}
ggplot(seed_stats_df, aes(x=seed, y=common_feats)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
```{r}
ggplot(seed_stats_df, aes(x=seed, y=jaccard)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
```{r}
ggplot(seed_stats_df, aes(x=seed, y=cor)) +
  geom_violin(alpha=0.5, fill="grey") +
  geom_boxplot(width=0.1, fill="grey") +
...
### Sampling Depth Experiments
```{r}
pm1 <- ggpubr::ggarrange(plotlist = lapply(list(p1, p3, p4, p5), function(p){p+theme(plot.margin = unit(c(1,1,1,1), "cm"))}), labels=c("A", "B", "C", "D"), ncol=2, nrow=2)
pm1
pfile <- "./results/publication/figures/insilico_sampling_depth_grid_plot.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = pm1, width = 15.5, height = 10, units = "in")
```

### Sampling Seed Experiments
```{r}
pm2 <- ggpubr::ggarrange(plotlist = lapply(list(p6, p8, p9, p10), function(p){p+theme(plot.margin = unit(c(1,1,1,1), "cm"))}), labels=c("A", "B", "C", "D"), ncol=2, nrow=2)
pm2
...
### Differential Experession Analysis Grid Plots
```{r}
pm2 <- ggpubr::ggarrange(plotlist = lapply(list(ptax6, p10_lm_de_vol, pca1p, pca2p), function(p){p+theme(plot.margin = unit(c(1,1,1,1), "cm"))}), labels=c("A", "B", "C", "D"), ncol=2, nrow=2)
pm2
pfile <- "./results/publication/figures/50K_lm_de_figure_3_grid.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = pm2, width = 17, height = 15, units = "in")
```

### 50K ROC and PR
```{r}
pm2 <- ggpubr::ggarrange(plotlist = lapply(list(roc_p, pr_p), function(p){p+theme(plot.margin = unit(c(1,1,1,1), "cm"))}), labels=c("A", "B"), ncol=2, nrow=1)
pm2
pfile <- "./results/publication/figures/50K_roc_pr_grid.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = pm2, width = 15, height = 8, units = "in")
```
...
runs_file <- "./data/all_runs.csv"
runs_dt <- data.table::fread(runs_file)
fwrite(runs_dt[ RunID %in% metadata_samples_keep$RunID ], "./data/run_accession_mapping.csv")
fwrite(merge(runs_dt[ RunID %in% metadata_samples_keep$RunID ], metadata_samples_keep, by=c("RunID", "ExptAcc")), "./data/run_accession_clinical_data.csv")
```
...
lm_dt %>%
  dplyr::filter( fdr < 0.01 ) %>%
  dplyr::arrange( fdr) %>%
  fwrite("./results/publication/tables/lm_de_significant_hits_at_0.01_fdr.csv")
...
lm_dt %>%
  dplyr::mutate()
ggplot() +
  geom_point(data=subset(lm_dt, fdr<0.01), aes(x=foldchange, y=-log10(fdr)), col="grey20" ) +
  geom_point(data=subset(lm_dt, fdr>0.01), aes(x=foldchange, y=-log10(fdr)), col="grey50" ) +
  geom_hline(yintercept = -log10(0.01), linetype="dashed") + 
  ggrepel::geom_text_repel(data=head(subset(lm_dt, fdr<0.001), 10), aes(x=foldchange, y=-log10(fdr), label=feature), box.padding = 0.5, max.overlaps = Inf) +
  ...
  theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        ...
        legend.title = element_text(size=16),
        legend.text = element_text(size=16)) +
  guides(fill="none") -> p10_lm_de_vol
p10_lm_de_vol
pfile <- "./results/publication/figures/lm_diff_exp_volcano.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = p10_lm_de_vol, width = 15, height = 10, units = "in")

# sig_feats <- c("Streptococcus salivarius", "[Clostridium] innocuum", "Collinsella aerofaciens", "Ruthenibacterium lactatiformans", "Streptococcus parasanguinis", "Streptococcus pneumoniae", "Escherichia coli" )
```

<!-- Create phyloseq object -->
```{r}
# Filter Freq data for only the identified significant features identified by lm
all_feats <- rownames(X_t)
# X_t <- X_t[ rownames(X_t) %in% sig_feats, ]
rownames(X_t) <- paste0("OTU", 1:nrow(X_t))
# Create species feature to phylum mapping table
if (T){
  feat_meta_phylum <- as.matrix(plyr::mapvalues(all_feats, from=taxonomy_phylum_db$query, to=taxonomy_phylum_db$name, warn_missing = FALSE))
  ...
  feat_meta_genus <- plyr::mapvalues(all_feats, from=taxonomy_db[ rank=="genus"]$query, to=taxonomy_db[ rank=="genus"]$name, warn_missing = FALSE)
  feat_meta_genus[feat_meta_genus %in% feat_meta_csv$feature] <- "Unknown"
  feat_meta_class <- plyr::mapvalues(all_feats, from=taxonomy_db[ rank=="class"]$query, to=taxonomy_db[ rank=="class"]$name, warn_missing = FALSE)
  feat_meta_class[feat_meta_class %in% feat_meta_csv$feature] <- "Unknown"
  feat_meta_order <- plyr::mapvalues(all_feats, from=taxonomy_db[ rank=="order"]$query, to=taxonomy_db[ rank=="order"]$name, warn_missing = FALSE)
  feat_meta_order[feat_meta_order %in% feat_meta_csv$feature] <- "Unknown"
  feat_meta_family <- plyr::mapvalues(all_feats, from=taxonomy_db[ rank=="family"]$query, to=taxonomy_db[ rank=="family"]$name, warn_missing = FALSE)
  feat_meta_family[feat_meta_family %in% feat_meta_csv$feature] <- "Unknown"
  feat_meta_phylum <- cbind(feat_meta_phylum, Class=feat_meta_class, Order=feat_meta_order, Family=feat_meta_family, Genus=feat_meta_genus, Species=all_feats)
} else {
...
phyloseq::plot_bar(physeq, fill = "Class") +
  ...
  theme(panel.background = element_blank(),
        ...
        legend.text = element_text(size=16),
        strip.text.x = element_text(size=18)) -> ptax2
ptax2
pfile <- "./results/publication/figures/50K_lm_0.01_fdr_class_classification.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax2, width = 11, height = 8, units = "in")
```
...
phyloseq::psmelt(physeq) %>%
  ...
  dplyr::mutate(Species = sub("\\s", "\n", Species)) %>%
  dplyr::mutate(row = ifelse( (Species %in% c("[Ruminococcus]\ngnavus", "Alloprevotella\nsp. E39", "Clostridioides\ndifficile")), 1, 2)) %>%
  ggplot(data = ., aes(x = ACVD, y = log2(Abundance))) +
  geom_boxplot(outlier.shape  = NA) +
  geom_jitter(col="grey30", height = 0, width = .2, alpha=0.1) +
  labs(x = "", y = "log2(Abundance)\n") +
  scale_x_discrete(labels= c(`0`="Healthy\nCohort", `1`="ACVD\nCohort")) +
  # facet_grid(row ~ Species, scales = "free") + 
  ...
  guides(color="none") -> ptax6
ptax6
pfile <- "./results/publication/figures/50K_lm_0.01_fdr_6_species_boxplot.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax6, width = 12, height = 8, units = "in")
...
phyloseq::psmelt(physeq) %>%
  ...
  labs(x = "", y = "Abundance\n") +
  scale_x_discrete(labels= c(`0`="Healthy Cohort", `1`="ACVD Cohort")) +
...
feat_meta_phylum %>%
  as.data.frame() %>%
  dplyr::filter( Species %in% sig_feats ) %>%
  dplyr::filter( Genus %in% "Bacteroides" ) %>%
  dplyr::pull(Species) -> sig_bacterioides

lm_dt %>%
  dplyr::filter( feature %in% sig_bacterioides) %>%
  dplyr::arrange(fdr)
  dplyr::select( Species=feature, pvalue, foldchange, fdr) %>%
  fwrite("./results/publication/tables/lm_de_significant_hits_at_0.01_fdr_bacterioides.csv")

phyloseq::psmelt(subset_taxa(physeq, Genus == "Bacteroides")) %>%
  dplyr::filter(Species %in% sig_bacterioides) %>%
  dplyr::filter(Abundance!=0) %>%
  dplyr::filter(!is.na(Abundance)) %>%
  ggplot(data = ., aes(x = ACVD, y = log2(Abundance))) +
  # ggplot(data = ., aes(fill = ACVD, x = log2(Abundance))) +
  ...
  facet_wrap(~ Species, scales = "free") +
  # facet_wrap( Species ~ Gender, scales = "free") +
  theme(axis.title = element_text(size=16),
        axis.text = element_text(size=15),
        legend.title = element_text(size=15),
        legend.text = element_text(size=14),
        strip.text.x = element_text(size=15)) +
  guides(fill="none") -> ptax8
ptax8
pfile <- "./results/publication/figures/50K_lm_0.01_fdr_bacterioides_species_boxplot.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax8, width = 15, height = 10, units = "in")
...
feat_meta_phylum %>%
  ...
  # dplyr::filter( !(Species %in% sig_feats) ) %>%
  dplyr::filter( Genus %in% "Bacteroides" ) %>%
  dplyr::pull(Species) -> total_bacterioides
...
feat_meta_phylum %>%
  ...
  dplyr::filter( !(Species %in% sig_feats) ) %>%
  dplyr::filter( Genus %in% "Bacteroides" ) %>%
  dplyr::pull(Species) -> non_sig_bacterioides

phyloseq::psmelt(subset_taxa(physeq, Genus == "Bacteroides")) %>%
  # dplyr::filter(Species %in% sig_bacterioides[c(1,2,5)]) %>%
  dplyr::filter(Abundance!=0) %>%
  ggplot(data = ., aes(x = ACVD, y = log2(Abundance))) +
  # ggplot(data = ., aes(fill = ACVD, x = log2(Abundance))) +
  # geom_density(alpha=0.3) +
  # geom_boxplot(outlier.shape  = NA) +
  # geom_jitter(aes(color = Species), height = 0, width = .2, alpha=0.1) +
  geom_violin(aes(fill = ACVD), alpha=0.5) +
  geom_boxplot(width=0.1) +
  labs(x = "", y = "Abundance\n") +
  scale_x_discrete(labels= c(`0`="Healthy\nCohort", `1`="ACVD\nCohort")) +
  # facet_wrap(~ Genus, scales = "free") +
  ...
  guides(fill="none") -> ptax8
ptax8
pfile <- "./results/publication/figures/50K_non_sig_bacterioides_species_boxplot.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax8, width = 15, height = 10, units = "in")
```
...
```{r}
phyloseq::plot_bar(subset_taxa(physeq, Genus == "Bacteroides"), fill = "Genus") +
  geom_bar(aes(color = Genus, fill = Genus), stat = "identity", position = "stack") +
  labs(x = "", y = "Relative Abundance\n") +
  facet_wrap(~ ACVD, scales = "free_x", labeller=as_labeller(acvd_label_map)) +
  theme(panel.background = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) -> ptax9
ptax9
pfile <- "./results/publication/figures/50K_lm_0.01_fdr_genus_bacterioides_classification.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax9, width = 15, height = 10, units = "in")
```

## Differential Expression Poisson Regression
```{r}
# Transpose and convert to matrix
X_t <- as.matrix(t(X))
colnames(X_t) <- metadata_samples_keep$RunID
...
lm_prep %>%
  dplyr::group_by(feature) %>%
  dplyr::group_map(~ do_lm(dt_sub = .x, grouped_var = .y, use_model = glm, family = "quasipoisson"), .keep=TRUE) %>%
  data.table::rbindlist() -> lm_dt
...
# remove nas
lm_dt2 <- lm_dt2[ !is.na(lm_dt2$pvalue), ]

hist(lm_dt2$pvalue, )

# Multiple Testing correction
lm_dt2$fdr <- p.adjust(lm_dt2$pvalue, method="fdr")
...
ggplot(lm_dt2 , aes(x=foldchange, y=-log10(fdr))) +
  geom_point()
...
phyloseq::plot_bar(physeq, fill = "Phylum") +
  ...
  theme(panel.background = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) -> ptax1_quasipoisson
ptax1_quasipoisson
pfile <- "./results/publication/figures/50K_quasipoisson_0.01_fdr_phylum_classification.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax1_quasipoisson, width = 15, height = 10, units = "in")
```
...
phyloseq::plot_bar(physeq, fill = "Class") +
  ...
  theme(panel.background = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) -> ptax2_quasipoisson
ptax2_quasipoisson
pfile <- "./results/publication/figures/50K_quasipoisson_0.01_fdr_class_classification.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax2_quasipoisson, width = 15, height = 10, units = "in")
```
...
phyloseq::plot_bar(physeq, fill = "Order") +
  ...
  theme(panel.background = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) -> ptax3_quasipoisson
ptax3_quasipoisson
pfile <- "./results/publication/figures/50K_quasipoisson_0.01_fdr_order_classification.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax3_quasipoisson, width = 15, height = 10, units = "in")
```
...
phyloseq::plot_bar(physeq, fill = "Family") +
  ...
  theme(panel.background = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) -> ptax4_quasipoisson
ptax4_quasipoisson
pfile <- "./results/publication/figures/50K_quasipoisson_0.01_fdr_family_classification.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax4_quasipoisson, width = 15, height = 10, units = "in")
```
...
phyloseq::plot_bar(physeq, fill = "Genus") +
  ...
  theme(panel.background = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) -> ptax5_quasipoisson
ptax5_quasipoisson
pfile <- "./results/publication/figures/50K_quasipoisson_0.01_fdr_genus_classification.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax5_quasipoisson, width = 15, height = 10, units = "in")
```
...
phyloseq::psmelt(physeq) %>%
  ...
  geom_jitter(aes(color = Species), height = 0, width = .2, alpha=0.1) +
  labs(x = "", y = "Abundance\n") +
  facet_wrap(~ Species, scales = "free") -> ptax6_quasipoisson
ptax6_quasipoisson
pfile <- "./results/publication/figures/50K_quasipoisson_0.01_fdr_6_species_boxplot.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax6_quasipoisson, width = 15, height = 10, units = "in")
...
phyloseq::psmelt(physeq) %>%
  ...
  geom_jitter(aes(color = Species), height = 0, width = .2, alpha=0.1) +
  labs(x = "", y = "Abundance\n") +
  facet_wrap(~ Species, scales = "free") -> ptax7_quasipoisson
ptax7_quasipoisson
pfile <- "./results/publication/figures/50K_quasipoisson_0.01_fdr_6_species_boxplot2.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = ptax7_quasipoisson, width = 15, height = 10, units = "in")
```
...
ggplot() +
  ...
  geom_line(data=roc_pdata_mean, aes(x=mean_fpr, y=mean_tpr-sd_tpr), col="#0c0c0c", alpha=0.4) +
  geom_line(data=roc_pdata_mean, aes(x=mean_fpr, y=mean_tpr+sd_tpr), col="#0c0c0c", alpha=0.4) +
  geom_ribbon(data=roc_pdata_mean,  aes(x=mean_fpr, y=mean_tpr+sd_tpr, xmin=min(mean_fpr), xmax=max(mean_fpr), ymin=mean_tpr-sd_tpr, ymax=mean_tpr+sd_tpr), fill="#0c0c0c", alpha=0.4) +
  ...
  scale_color_manual(labels =  
                     ...
                     values = c('#f3f3f3', '#dcdcdc', '#c5c5c5', '#aeaeae', '#979797', '#808080', '#686868', '#515151', '#3a3a3a', '#232323', "#0c0c0c")) +
...
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = roc_p, width = 9, height = 6, units = "in")
```
...
ggplot() +
  ...
  geom_line(data=pr_pdata_mean, aes(x=mean_rec, y=mean_prec-sd_prec), col="0c0c0c", alpha=0.4) +
  geom_line(data=pr_pdata_mean, aes(x=mean_rec, y=ifelse(mean_prec+sd_prec>1, 1, mean_prec+sd_prec)), col="0c0c0c", alpha=0.4) +
  geom_ribbon(data=pr_pdata_mean,  aes(x=mean_rec, y=mean_prec, xmin=min(mean_rec), xmax=max(mean_rec), ymin=mean_prec-sd_prec, ymax=ifelse(mean_prec+sd_prec>1, 1, mean_prec+sd_prec)), fill="0c0c0c", alpha=0.4) +
  geom_abline(intercept=1, slope=-1, col="black", linetype="dotted") +
...
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = pr_p, width = 9, height = 6, units = "in")
```
...
pca_dt$`ACVD status` <- plyr::mapvalues(pca_dt$`ACVD status`, from = c(0, 1), to = c("Healthy", "ACVD"))
...
ggplot(pca_dt, aes(x=PC1, y=PC2, col=`ACVD status`, shape=`ACVD status`)) +
  geom_point() +
  scale_color_manual(labels = c("Healthy", "ACVD"), 
                     values = c("Healthy"="#bfbfbf", "ACVD"="#404040")) +
  labs(x=sprintf("PC1 (%s%%)", round(total_var_explained[1],4)*100), y=sprintf("PC2 (%s%%)", round(total_var_explained[2],4)*100)) +
  theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        axis.title = element_text(size=18),
        axis.text = element_text(size=17),
        legend.title = element_text(size=18),
        legend.text = element_text(size=17)) -> pca1p
pca1p
pfile <- "./results/publication/figures/50K_pca1p.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = pca1p, width = 10, height = 7, units = "in")

# Generate Plot for PC2-PC3
ggplot(pca_dt, aes(x=PC3, y=PC2, col=`ACVD status`, shape=`ACVD status`)) +
  geom_point() +
  scale_color_manual(labels = c("Healthy", "ACVD"), 
                     values = c("Healthy"="#bfbfbf", "ACVD"="#404040")) +
  ...
  theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        ...
        axis.text = element_text(size=17),
        legend.title = element_text(size=18),
        legend.text = element_text(size=17)) -> pca2p
pca2p
pfile <- "./results/publication/figures/50K_pca2p.png"
message(sprintf("Saved figure as: %s", pfile))
ggsave(pfile, plot = pca2p, width = 10, height = 7, units = "in")

# Generate Plot for PC3-PC4
ggplot(pca_dt, aes(x=PC3, y=PC4, col=`ACVD status`, shape=`ACVD status`)) +
  geom_point() +
  scale_color_manual(labels = c("Healthy", "ACVD"), 
                     values = c("Healthy"="#bfbfbf", "ACVD"="#404040")) +
...
# Expeirment and Accessions Used Table
```{r}
freq_matrix_50k <- fread("./workflow/PE_subsampling_experiments/insilico_depth_experiment/kraken2_PE_50K/matrix/freq_mat.csv")
sra_accession_file <- fread("./data/all_runs.csv")
clinical_paramters_file <- fread("./data/Jie_2017_supp_table_1.csv")

used_accessions <- gsub( "^(ERR\\d+)_.*" ,"\\1", freq_matrix_50k$filename)

sra_accession_file %>%
  dplyr::filter(RunID %in% used_accessions) %>%
  dplyr::select(RunID, ExptAcc, `Sample ID`=Alias) %>%
  merge(clinical_paramters_file, by="Sample ID") %>%
  fwrite("./results/publication/tables/table_1.csv")
